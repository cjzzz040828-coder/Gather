<template>
	<view class="page">
		...
		<view class="nav-bar">
			...
			<view class="nav-right" @tap="goContacts"><u-icon name="account" color="#07C160" size="22"></u-icon></view>
		</view>

		<view class="search-bar">
			<view class="search-input" @tap="navigateToSearch">
				<u-icon name="search" color="rgba(0,0,0,0.35)" size="15"></u-icon>
				<text class="search-placeholder">搜索</text>
			</view>
			<view class="add-btn" @tap="showAddMenu = !showAddMenu">
				<u-icon name="plus-circle" color="#07C160" size="22"></u-icon>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goContacts() { uni.navigateTo({ url: '/pages/contacts/index' }) },
...
<style lang="scss" scoped>
	...
	.nav-bar { height: 44px; display: flex; align-items: center; position: relative; background: #fff; flex-shrink: 0; }
	...
	.nav-right { position: absolute; right: 12px; padding: 6px; }

	.search-bar { display: flex; align-items: center; padding: 12rpx 20rpx; background: #fff; gap: 12rpx; border-bottom: 1px solid #f0f0f0; flex-shrink: 0; }
	.search-input {
		flex: 1; display: flex; align-items: center; height: 60rpx;
		background: #f2f3f5; border-radius: 30rpx; padding: 0 16rpx;
	}
	.search-placeholder { font-size: 24rpx; color: rgba(0,0,0,0.35); margin-left: 8rpx; }
	...
	.menu-content {
		position: absolute; top: 180rpx; right: 20rpx;
